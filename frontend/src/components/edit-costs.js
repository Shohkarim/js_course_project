...
export class EditCosts {
    constructor() {
        ...
        if (!this.form || !this.input || !this.categoryId || this.categoryType !== 'expense') {
    ...
    async loadCategory() {
        try {
            const result = await CustomHttp.request(`${config.host}/categories/expense/${this.categoryId}`, 'GET');
    ...
    async updateCategory(event) {
        ...
        try {
            const result = await CustomHttp.request(`${config.host}/categories/expense/${this.categoryId}`, 'PUT', {
                title: title
            });
